# FreeSWITCH configure reference

This is captured `./configure --help` output from a historical FreeSWITCH
1.10.11 build. It is retained as a build reference; it is not the list of
options currently passed by the Dockerfile. The final line records the build
failure that accompanied the capture.

## Options used by this image

The current Dockerfile invokes `./configure` with:
...
## Historical `./configure --help` output
